...
_breaker_probing = False  # True khi đang có ĐÚNG 1 probe half-open chạy (single-flight)
...
def _breaker_allow():
    ...
    with _breaker_lock:
        if time.monotonic() < _breaker_open_until:
            return False                      # đang open trong cooldown -> chặn
        if _breaker_fails >= _BREAKER_FAILS:  # đã mở, cooldown hết -> giai đoạn half-open
            if _breaker_probing:
                return False                  # đã có probe đang chạy -> chặn phần còn lại
            _breaker_probing = True           # mình là probe duy nhất
            do_reset = True
    if do_reset:
        reset_pool(force=True)                # probe dùng socket MỚI, bỏ qua cooldown reset
    return True


def _breaker_record(ok):
    """Cập nhật breaker sau 1 call: ok=True -> đóng (reset); ok=False (connect-fail) -> đếm,
    đủ thì mở. Luôn nhả cờ probe để half-open kế tiếp được phép thử lại."""
    global _breaker_fails, _breaker_open_until, _breaker_probing
    with _breaker_lock:
...
class JiraAuthError(RuntimeError):
    """PAT chung sai/hết hạn/bị thu hồi. Subclass RuntimeError -> mọi `except RuntimeError`
    cũ vẫn bắt được (tương thích), nhưng caller muốn phân biệt 'PAT hỏng' với 'mất mạng'
    thì bắt riêng cái này. Dấu hiệu: HTTP 401, hoặc 400 'cannot be viewed by anonymous'
    (PAT chết -> Jira coi request là vô danh -> query đụng field reporter/assignee bị 400)."""
...
def reset_pool(force=False):
    ...
    with _pool_reset_lock:
        now = time.monotonic()
        if not force and now - _pool_reset_at < _POOL_RESET_COOLDOWN:
            return
        _pool_reset_at = now
    try:
...
def _cached_swr(key, producer, block=True, force=False):
    ...
    force=True (Decision #26 bổ sung 'F5 = luôn tươi'): BỎ QUA cache fresh/stale, tính đồng
    bộ ngay để lấy data mới nhất. Dùng khi user chủ động F5/hard-reload (không phải click
    chuyển tab). Với block=False (chuông), nếu Jira lỗi -> trả data cũ (nếu có) / sentinel
    thay vì raise (không kéo sập điều hướng)."""
    now = time.monotonic()
    with _cache_lock:
        entry = _cache.get(key)
    ...
    try:
        data = producer()                     # force / miss / quá-cũ -> đồng bộ
    except Exception:
        if not block:                         # best-effort (chuông): Jira lỗi -> data cũ/sentinel
            return entry[0] if entry is not None else _SWR_NOT_READY
        raise                                 # đường block: raise như cũ -> trang lỗi
    _cache_set(key, data)
    return data
...
def fetch_all(scope_user=None, force=False):
