...
Lưu trữ (`.bug_log.json` local = full; Jira property = full hoặc bản nhẹ nếu > ~32KB):
  {
    ...
    "activity": [ {id,key,summary,by,author,when,new,kind,file,sheet}, ... ],   # cap + prune
    "synced_at": iso
  }
...
def _diff_events(prev_bugs, cur_bugs, file_name=''):
    ...
    for key, b in prev_bugs.items():
        if key not in cur_bugs:
            ...
            events.append({
                ...
                'by': author, 'author': author or '?', 'when': iso,
                'new': f"xoá bug {label(b)}", 'kind': 'del',
                'file': file_name, 'sheet': (b.get('month') or '').strip(),
            })
    return events
...
def scan():
    ...
    with _scan_lock:
        ...
        if new_events:
            ...
            # Chi tiết cho popup "sau đồng bộ": nêu rõ file/sheet/nội dung đổi (cap 80 dòng
            # để payload JSON gọn; new_events đã mới-nhất-trước).
            result['changes'] = [{
