...
from custom_status import values_of  # noqa: E402
...
def _mock(has_pat, seed='{}'):
    ...
    `seed` = JSON {key: [nhãn...]} khởi tạo state custom-status để /set-custom-status
    TOGGLE đúng như server (trả `values`) -> multi-select bấm thử y hệt production."""
    if has_pat:
    ...
    # toggle in-memory giống set_custom_status server: thêm nếu chưa có, bớt nếu đang có, ''=gỡ hết
    cust = (
        "else if(url.indexOf('/set-custom-status')>=0){"
        "var b=JSON.parse(opts.body||'{}');"
        "var cur=(window.__cust[b.key]||[]).slice();"
        "if(b.status===''){cur=[];}"
        "else{var i=cur.indexOf(b.status);if(i<0)cur.push(b.status);else cur.splice(i,1);}"
        "window.__cust[b.key]=cur;data={ok:true,values:cur};}"
    )
    return ("<script>(function(){"
            f"window.__cust={seed};"
            "window.fetch=function(url,opts){var data={ok:true};"
...
def _patch(html, home_file, has_pat=True, extra='', seed='{}'):
    html = html.replace('<body>', '<body>' + _mock(has_pat, seed), 1)
...
def main():
    ...
    # DA51H26-101: nhiều nhãn (demo multi-select); DA61H26-210: format cũ string (demo migration-safe)
    overlay = {'DA51H26-101': {'v': ['dev_fixing', 'wait_ba'], 'by': 'quangbm', 'at': _iso(_NOW)},
    ...
    hp = _hint('QA (CÓ PAT): bấm ▾ → "Custom status" tick được NHIỀU nhãn (menu giữ mở, badge gộp ● A, B). '
               'DA51H26-101 sẵn 2 nhãn. "Status Jira" chỉ chọn 1 (fetch động, đổi Jira thật).')
    np = _hint('QA (CHƯA PAT): "Status Jira" hiện 🔒 + Comment → popup. '
               'NHƯNG custom status (●) VẪN tick nhiều nhãn bình thường (không cần PAT).')

    files = {
        'preview.html': _patch(admin, 'preview.html', has_pat=True, seed=seed),
        'preview_personal.html': _patch(personal, 'preview_personal.html', has_pat=True, extra=hp, seed=seed),
        'preview_personal_nopat.html': _patch(personal, 'preview_personal_nopat.html', has_pat=False, extra=np, seed=seed),
        'preview_settings.html': _patch(settings, 'preview_personal.html', has_pat=True, seed=seed),
