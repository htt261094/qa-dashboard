...
- **Cold-load fix (2026-06-22)**: model 12B nặng 8GB → Ollama default `keep_alive=5m` unload sau 5p idle → request kế chịu **~30s nạp lại disk→GPU** (im lặng, chưa quay quạt) rồi vòng đầu `stream=False` dễ vấp lỗi 500/timeout → "chatbot dừng response" (1 lần/idle). Sửa 2 phần: (a) `config.OLLAMA_KEEP_ALIVE` (env, default `'-1'` = giữ thường trú mãi; `chat._parse_keep_alive` → int hoặc chuỗi `'30m'`), truyền `keep_alive` vào MỌI payload `/api/chat` (gồm pre-warm) → `ollama ps` báo `UNTIL Forever`. (b) `chat.prewarm()` — `main()` spawn daemon thread lúc khởi động gửi 1 ping `num_predict:1` nạp model trước, soft-fail (Ollama chưa sẵn/model chưa pull → bỏ qua êm). Kết quả: chỉ lần nạp đầu sau khi (re)start Ollama mới tốn 30s, ẩn trong startup; user không bao giờ chạm cold-load. ⚠ `-1` ghim ~8GB GPU vĩnh viễn (host chuyên dụng → chấp nhận; muốn nhả set `OLLAMA_KEEP_ALIVE=30m`). Warm first-byte ~4.4s (prompt nhỏ); 12B vẫn nặng hơn e4b nên prefill prompt lớn (summary+scrape) lâu hơn.
