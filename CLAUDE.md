...
### 25. Reopen Bug Log chuyển sang SQLite (`bug_log_db.py`) — bền qua đổi host (2026-06-12, option A)
- **Bối cảnh / vì sao**: metric "Tỷ lệ Reopen" báo 0% dù 7 bug DA6 đang ở `Reopen` (vd DA6-28 reopen hôm qua). Nguyên nhân: counter reopen cũ (`_count_reopens`) **diff theo snapshot JSON** trong `.bug_log.json`. Snapshot đó (a) bị `_sync_property` strip thành bản `_light` khi data > ~32KB (bug log 843KB → property KHÔNG mang `bugs`), (b) **reset baseline khi đổi host Mac→Windows** → lần scan đầu trên host mới thấy MỌI bug là "mới" (activity 200 event "log bug" cùng 1 mốc 2026-06-12T08:38 = smoking gun) → KHÔNG chứng kiến transition `→Reopen` → counter rỗng. SUPERSEDES `_count_reopens` (đã xoá).
- **Quyết định (option A, user chốt)**: vòng đời reopen lưu **SQLite `.bug_log.db`** (`config.BUG_LOG_DB`, root, gitignore) — `sqlite3` stdlib, KHÔNG vi phạm minimal-deps. 3 bảng: `bug_status` (trạng thái gần nhất mỗi bug = baseline diff, BỀN không bị property strip), `status_event` (append-only mỗi lần status đổi — audit + recompute được), `reopen_state` (counter {count,fix,...}). DB chỉ là **kho local bền**; reopen counts VẪN mirror lên Jira property qua `data['reopen']` (nhỏ, vừa 32KB) → cross-host **đọc** được mà KHÔNG đụng Jira nghiệp vụ.
- **Module `bug_log_db.py`** (layer `config → (this)`, không cycle): `apply_file(fid, cur_bugs)` diff vs baseline DB → ghi event + cập nhật reopen_state + upsert baseline; `import_reopen(map)` nạp counts từ property-mirror vào DB cho host mới (idempotent, KHÔNG ghi đè key đã có); `reopen_dict()` export shape `reopen` cũ (`{key:{count,fix,last,dev,project,month}}`) → render client-side + property mirror KHÔNG đổi.
- **Wiring `bug_log_store.scan()`**: trước loop `import_reopen(reopen)`; trong loop thay `_count_reopens(...)` bằng `bug_log_db.apply_file(fid, cur_bugs)`; sau loop `data['reopen'] = bug_log_db.reopen_dict()`. `_diff_events` (activity feed) GIỮ diff theo JSON như cũ — chỉ reopen chuyển DB. Tầng-1 skip (file không đổi) → apply_file không chạy: OK vì không có transition mới; lần deploy đầu cache còn `_version=2` ≠ 3 → ép re-scan → seed chạy.
- **Seed (user chốt)**: bug LẦN ĐẦU thấy trong DB mà đang `Reopen` và chưa có reopen_state → `count=1, fix=1`. Bắt được reopen "có sẵn" lúc bật/đổi host (gồm 7 bug hiện tại) thay vì mù tới reopen MỚI. **Hệ quả số liệu**: per-dev "Tỷ lệ reopen" của bug seeded = 1/1 = **100%** tới khi dev fix lại (→50%…) — đây là biểu diễn trung thực "chưa có lịch sử fix", tự sửa qua các vòng witnessed. Headline KPI (`distinctTotal/totalBugs` = % bug bị reopen) KHÔNG bị ảnh hưởng.
- **Đổi host**: counts tự sống qua property mirror; muốn giữ FULL baseline + lịch sử event thì copy `.bug_log.db`. **Giới hạn cố hữu (SQLite KHÔNG sửa)**: vẫn lấy mẫu mỗi `POLL_SECONDS` → round-trip Fixed→Reopen→Fixed gọn trong 1 cửa sổ vẫn sót.
- **⚠ Cần restart server bằng code branch này** để fix có hiệu lực; lần scan đầu sẽ seed 7 bug Reopen hiện tại.
