...
- **Row hover actions**: admin — `.row-acts` overlay góc phải cell Updated (absolute, hiện khi tr:hover): mở drawer (`data-act="row-detail"` — bubble vào tr handler sẵn có) + mở Jira. QA — thêm nút "Xem chi tiết" cạnh nút bình luận (delegation data-act="detail" sẵn có). Chỉ markup + CSS, 0 handler mới.
