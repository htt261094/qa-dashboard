...
### 60. Cổng QA gate — cảnh báo task cha READY PRODUCTION mà THIẾU sub-task QA (2026-07-24)
- **Bối cảnh / vì sao**: sắp tính story point cho QA. Có kịch bản sai quy trình: task cha (dev) được chuyển sang trạng thái `READY PRODUCTION` **trong khi list sub-task chưa có sub-task QA nào** → QA chuyển trạng thái TRƯỚC, tạo sub-task SAU (né/làm lệch việc ghi nhận story point). User cần **notify để nhắc**. User chốt (AskUserQuestion): status **`READY PRODUCTION`** (all-caps, đúng case) nằm trên **task CHA (dev)**; "sub-task QA" = summary bắt đầu `[QA]` **và assignee ∈ team QA (USERS)**; QA có quyền là người chuyển trạng thái; quét **TOÀN instance**; hiển thị **đẩy vào chuông notification**.
- **Phát hiện** (`jira_api._compute_ready_prod_gaps`, cache SWR qua `fetch_ready_prod_gaps(force)`): (1) JQL `status = "READY PRODUCTION" [AND project in (...)] ORDER BY updated DESC` (cap `READY_PROD_MAX=150`, `expand=changelog`); loại issue bản thân là sub-task (`issuetype.subtask`). (2) `_subtasks_of_parents` lấy sub-task theo lô 50 qua `parent in (...)` (fallback `parent =` từng cha nếu Jira không hỗ trợ `in`). (3) task cha **không có** sub-task nào thoả `_is_qa_subtask` (summary `[QA]*` + assignee ∈ USERS) → vi phạm. `_last_transition_to` lấy **ai + khi nào** chuyển sang READY PRODUCTION từ changelog. **CHỈ báo khi NGƯỜI CHUYỂN cũng là QA (∈ USERS)** — đúng kịch bản "QA chuyển trạng thái trước, tạo sub-task sau"; dev/người ngoài team hoặc không rõ tác giả (ngoài cửa sổ changelog) → bỏ qua. Mỗi vi phạm = activity dict `{id:'<KEY>#qa-gate', kind:'qa_missing', key, summary, author, author_user, new:READY_PROD_STATUS, when}`. id ổn định → dismiss được + không toast lại; sửa (thêm sub-task QA) → tự biến mất.
- **Best-effort, KHÔNG treo chuông**: `fetch_ready_prod_gaps` OFFLINE→[]; SWR `block=False` (miss→[] + refresh nền, Jira lỗi→nuốt) nên KHÔNG bao giờ raise/treo. Quét toàn instance = **giống nhau mọi người** → cache dùng chung (không scope), caller lọc sau. Cost: 1 call changelog (cap 150) + 1-vài call subtask, ~mỗi 2 phút (SWR TTL) chứ không mỗi poll/tab.
- **Wiring chuông** (`qa_dashboard`): thêm job `'gaps'` vào `run_parallel` ở CẢ `_bell_activities` (poll `/activity-feed` + tab my-work/docs/roadmap...) LẪN canonical `/` (`_get_dashboard`). Gaps trộn vào `merged` **SAU `_drop_own_activities`** (chủ đích KHÔNG lọc bỏ dù chính QA gây ra — đó là người cần nhắc). **Scope**: admin (`scope is None`) thấy hết; QA/dev chỉ thấy gap do **chính họ** chuyển (`author_user == scope`) → không spam cả team. Rồi mới gán `is_unread`.
- **Client** (`app_v2.js` bell): thêm `qa_missing` vào `KIND_IC` (icon `warning`), `kindCls`→`k-warn`, `ntext` (`"<author> chuyển <KEY> → READY PRODUCTION nhưng CHƯA có sub-task QA"`, status lấy từ `n.new` để không hardcode), `rsn`→"Nhắc quy trình". Bấm noti mở drawer task cha (`__openDetail` — task cha ngoài bucket vẫn synth được, Decision #18). CSS `.nkind.k-warn`(đỏ `--error`) + `.nrsn.warn` trong `styles_v2.css`.
- **Config** (`config.py` + `.env.example`): `READY_PROD_STATUS` (default `READY PRODUCTION`), `READY_PROD_PROJECTS` (comma-sep, rỗng=toàn instance), `READY_PROD_MAX` (clamp [1,300], default 150).
- **Giới hạn**: (a) toàn instance → nếu READY PRODUCTION dùng ở project dev KHÔNG có quy trình QA sẽ báo dương tính giả — set `READY_PROD_PROJECTS` để thu hẹp mà không sửa code; (b) `_is_qa_subtask` = prefix `[QA]` + **assignee ∈ USERS** (chỉ tính người làm là QA; assign cho dev/người ngoài team KHÔNG cứu task cha khỏi bị flag). Thêm QA mới phải cập nhật `JIRA_USERS`; (c) trễ tối đa ~2 phút (SWR TTL); (d) đổi trạng thái ngoài cửa sổ changelog retention → không xác định được tác giả → bỏ qua HOÀN TOÀN (kể cả admin) vì actor-must-be-QA; đây là đánh đổi để tránh dương tính giả trên task do dev chuyển. Cần **restart app** (config + jira_api + qa_dashboard Python); JS/CSS hot-reload F5. Chưa chạy live (cần VPN+PAT + task READY PRODUCTION thật).

### 61. PLACEHOLDER chuyển nguồn Bug Log Google Sheet → Jira (kiến trúc provider + UI metric rỗng) (2026-07-24)
- **Bối cảnh / vì sao**: sắp tới team sẽ **log bug trực tiếp trên Jira** thay vì Google Sheet như hiện tại. User muốn làm TRƯỚC placeholder để khi Jira bug logging được công bố là cắm nguồn vào ngay + làm thêm metric mà quản lý bug trên Jira đo được (thời gian khắc phục, tuổi bug, priority, throughput...). User chốt (AskUserQuestion): **mô hình Jira CHƯA chốt** → làm lớp nguồn TRUNG TÍNH; **phạm vi đợt này** = khung nguồn + toggle **và** UI metric rỗng render "Chờ dữ liệu Jira" để chốt layout trước. **CHƯA gọi Jira thật.**
- **Seam**: mọi tầng downstream chỉ phụ thuộc **`bug` dict** (`bug_log.normalize`) + `.bug_log.json` do `scan()` ghi. Chỗ route nguồn duy nhất = `bug_log_store._scan_one(src, prev)`. Placeholder chèn tại đúng seam này → downstream (store/backlog/task_link/render/analytics) KHÔNG đổi.
- **A — Source trung tính** (`bug_log_source.py`): mỗi source thêm field optional `provider` ∈ `{'drive','jira'}`, thiếu = `'drive'` (backward-compat tuyệt đối). Helper `provider_of(src)`. `valid_sources` nới cho jira: `id` chuỗi non-empty ≤500 (project key/JQL ngắn, KHÔNG ép `_ID_RE` Drive) + field optional `query` ≤2000; drive giữ nguyên `_ID_RE`. Provider lạ → invalid.
- **B — Module stub `core/bug_source_jira.py`** (layer `config` → this; lazy-import `jira_api` khi fetch thật): `scan_source(src)` trả `{bugs,unmapped,meta,pending,error}` — cùng hình dạng `_scan_one` cần. Gate `config.BUG_LOG_JIRA_ENABLED` (env, default False): **tắt → `{bugs:[],pending:True}` KHÔNG gọi mạng**. `_issue_to_bug`/`_fetch_issues` = `NotImplementedError` (scaffold) + docstring **bảng mapping Jira field → 13 key bug dict** để điền sau. Ghi chú: khi bật, cân nhắc dùng **Jira issue key** làm định danh bền → có thể BỎ logic carry/copy-sheet (fingerprint/`_dedup_by_fp`/`backfill_fingerprints`/`_seed_current_reopens`) vốn chỉ tồn tại vì Sheet thiếu ID ổn định.
- **C — Dispatch `_scan_one`** (`bug_log_store.py`): đầu hàm `provider_of(src)=='jira'` → lazy-import `bug_source_jira.scan_source`. `pending`/rỗng → `{unchanged:True}` = **no-op** (không đụng store); có bug (tương lai) → dựng `cur_bugs` giống nhánh Drive → merge/diff/reopen dùng lại toàn bộ. Nhánh Drive y nguyên.
- **D — UI metric placeholder** (`render/analytics.py`): section "Metric từ Jira (sắp có)" cuối trang `/analytics` = 6 card empty-state "⏳ Chờ dữ liệu bug từ Jira" (data-driven từ list `_JIRA_METRICS`): thời gian khắc phục · tuổi bug đang mở · theo priority · thời gian phản hồi đầu · throughput tuần · theo component. Embed `jiraMetrics:{}` vào `analyticsData` làm hook; controller `#analyticsData` có `renderJiraMetrics()` no-op (TODO). Metric hiện có KHÔNG đổi. CSS tái dùng `.empty-state`/`.es-*` (#38) + thêm `.soon-badge`/`.jira-soon`.
- **Config** (`config.py` + `.env.example`): `BUG_LOG_JIRA_ENABLED` (bool, default False, nhận 1/true/yes).
- **KHÔNG làm đợt này** (chốt sau khi model Jira công bố): UI thêm nguồn Jira trong modal quản lý (fields phụ thuộc model); fetch Jira thật + mapping status/priority; quyết định định danh bền (Jira key vs fingerprint).
- **Verify** (không mạng): `py_compile` config/bug_log_source/bug_log_store/render.analytics/bug_source_jira/qa_dashboard + `node --check app_v2.js` OK; smoke `valid_sources` (drive cũ OK, jira id lỏng + query OK, provider lạ/jira id rỗng/drive id ngắn bị từ chối), `provider_of` default drive, `scan_source` disabled → `{bugs:[],pending:True}` không gọi mạng, `_scan_one` jira → no-op `{unchanged:True}` không đụng store; render `/analytics` → đúng 6 card jira-soon + 6 empty-state + embed hook, metric cũ nguyên. Live browser `/analytics` KHÔNG chạy được ở đây (AUTH Google bật, cần session đăng nhập thật) — đã verify bằng server-side render (HTML browser nhận). Cần **restart app** (config + module Python mới); JS/CSS hot-reload F5.

### 62. Tồn đọng T-1 — bắt bản copy ĐÃ BÊ SANG sheet T dù bị đổi ngày created (2026-07-28)
- **Bối cảnh / vì sao**: chart báo "còn 1 bug T-1 chưa fix" (DA5-B2B #45, tạo 30/06 T6, status New) nhưng thực tế bug ĐÃ được bê sang sheet T7 (#47) và **Closed**. Fingerprint khớp hoàn hảo (T6#45 và T7#47 cùng `da5|b2b|<summary>`), bản T7 = Closed, đáng lẽ tính "đã xử lý". Gốc rễ = `prev_month_backlog`/`computeBacklog` gom nhóm tồn đọng **chỉ từ bug có `created[:7] == prev`** (tháng 6), rồi mới xét trong nhóm "có bản nào Closed + ở sheet T7". Team copy bug sang T7 nhưng **đổi ngày created bản copy sang 01/07/2026** (thay vì giữ ngày gốc T6 như giả định Decision #46) → bản Closed đó `created` tháng 7 → KHÔNG lọt vào nhóm tháng 6 → nhóm chỉ còn bản gốc New → tính "còn treo"; đồng thời bản copy bị đếm thành "bug mới phát sinh T7". SUPERSEDES giả định "bản copy giữ nguyên created T-1" của Decision #46.
- **Fix (hướng B, 2 nơi parity)**: thêm index `carried_by_fp` = bug **cùng fingerprint có `month == cur_sheet`** (vd `T7`), **BẤT KỂ `created`**. Nhóm tồn đọng vẫn định nghĩa bằng bug created T-1 (`groups`), nhưng khi xét `any_closed`/`carried` thì gộp `members = group + carried_by_fp[fp]` → bắt được bản đã bê sang T dù đổi ngày. `carried = bool(carried_by_fp[fp])`. Sửa `core/bug_backlog.py` `prev_month_backlog` + twin JS `computeBacklog` ([app_v2.js](assets/app_v2.js)) — logic y hệt để 2 phía ra cùng số.
- **Vì sao CHỈ nhặt bản ở đúng sheet T (không phải mọi bug cùng fp toàn cục)**: tránh kéo nhầm bug cũ tháng khác (vd bản Closed ở T5 có summary trùng) vào diện "đã đóng" → sẽ giấu oan 1 bug T-1 đang thật sự treo. `month == cur_sheet` là tín hiệu "đã carried sang tháng T" chính xác.
- **KHÔNG đụng**: định nghĩa `fingerprint` (Decision #54); `new_count` vẫn = unique-fp created trong report_month → **bản copy đổi ngày sang T7 vẫn bị đếm là "mới phát sinh T7"** (vấn đề riêng, user chưa yêu cầu sửa — nếu cần thì loại bug mà fp đã thuộc tồn đọng T-1 khỏi new_count). Chart freeze tháng đã đóng (Decision #47) KHÔNG bump `_CHART_V` (tồn đọng report tính LIVE, tự đúng khi F5).
- **Verify** (không mạng): `py_compile bug_backlog` + `node --check app_v2.js` OK; smoke `prev_month_backlog('2026-07')` trên `.bug_log.json` thật → still_open **1→0**, resolved 50→51 (T6#45 giờ khớp T7#47 Closed); **parity** JS `computeBacklog` cùng data = y hệt `{total:51, stillOpen:0, resolved:51, newCount:88}`. Chart tự đúng khi F5 (JS hot-reload); report Google Chat cần **restart app**.

### 63. Xem tài liệu NGAY trong app — viewer inline thay vì mở tab mới (2026-07-30)
- **Bối cảnh / vì sao**: `/docs` cũ bấm tài liệu là `window.open(url,'_blank')` — mọi file (kể cả file đã upload lên host) đều nhảy tab mới. User yêu cầu preview ngay trong hệ thống; chốt **làm hết** (PDF/ảnh + docx/xlsx/pptx + link Drive).
- **Viewer** = overlay `#fpOverlay` (`render/docs.py`, chỉ trang Tài liệu) + controller trong closure `#docsData` (`app_v2.js`): `openDocPreview(doc)` chọn nhánh theo đuôi file/URL. Header có tên file + nút **Tải xuống** (chỉ file local) + **Mở tab mới** (giữ đường thoát khi preview không đủ) + ×; đóng bằng ×/click nền/Esc (`stopPropagation` + capture để không đụng handler Esc khác). Đóng thì **xoá `innerHTML`** → gỡ iframe, dừng tải. Bấm dòng trong bảng (`data-id` → `findFileById`) hoặc menu ngữ cảnh "Xem trước"; menu giữ thêm "Mở tab mới".
- **4 nhánh render**: (a) `/uploads/*.pdf` → `<iframe src=...#view=FitH>` (PDF viewer sẵn của browser); (b) ảnh → `<img>`; (c) docx/xlsx/pptx/text → `GET /file-preview?f=<filename>` trả HTML dựng server-side; (d) link Google → iframe bản `/preview` (`fpDriveEmbed` map `docs.google.com/{document,spreadsheets,presentation}/d/<id>` + `drive.google.com/file/d/<id>` + `open?id=` → `/preview`; không map được → empty-state + nút mở tab mới). Drive nhúng được vì **browser của user đã đăng nhập Google** — server KHÔNG chạm Drive API.
- **`core/file_preview.py`** (zero-dep, stdlib): xlsx/xlsm → tái dùng `bug_log.list_sheet_names` + `read_sheet_rows` (KHÔNG thêm openpyxl) dựng bảng mọi sheet; docx → zip + regex `word/document.xml` (đoạn văn + Heading→`<h2..h6>` + `<w:tbl>` thành bảng, giữ đúng thứ tự); pptx → text từng slide (`<a:t>`); text/csv/tsv/json/md/log → `<pre>` (csv/tsv thành bảng). **MỌI nội dung qua `issues.esc`** (file do người dùng up = untrusted → chống stored XSS). Cap: 25MB/file · 500 dòng/bảng · 40 cột · 800 đoạn · 400k ký tự, vượt thì cắt + ghi chú. KHÔNG raise: lỗi parse → `ok=False` + thông báo tiếng Việt.
  - ⚠ Regex `<w:t[^>]*>` khớp NHẦM `<w:tc>` (ô bảng) làm lọt markup thô vào text — đã dính lúc verify. Phải là `<w:t(?:\s[^>]*)?>`; áp cùng kiểu cho `w:tbl/w:tr/w:tc`.
- **Route** `GET /file-preview` (`UploadsMixin._get_file_preview`): `basename(unquote(f))` + check `resolve().parent == UPLOADS_DIR.resolve()` → chỉ đọc trong uploads. Gated authed/domain như mọi GET; KHÔNG trong dev allowlist (dev không vào `/docs`).
- **Giới hạn**: preview Office là **bản gần đúng để đọc nhanh** — mất font/màu/merge cell phức tạp, ảnh nhúng trong file bị bỏ; cần bản gốc thì Tải xuống. `.doc/.xls/.ppt` (định dạng cũ) + `.zip` → empty-state. Drive folder không nhúng được. Cần **restart app** (route + module + template Python); JS/CSS hot-reload F5.
- **Verify** (live browser, harness tĩnh dựng từ `render_docs_page` thật + endpoint thật, port 8099): xlsx 12 dòng ra bảng đúng, docx ra heading/đoạn/bảng đúng (không lọt markup, `<script>` bị escape), txt ra `<pre>`, png ra `<img>`, link Drive ra iframe `/preview` + ẩn nút Tải xuống, Esc đóng + body rỗng, **0 lỗi console**; unit: traversal `../.env` bị chặn, file hỏng/thiếu → thông báo, `.zip` → unsupported.

### 64. Test Case đọc cột Automated + Automation Result — metric độ phủ automation (2026-07-30)
- **Bối cảnh / vì sao**: sheet test case bổ sung 2 cột. **Automated** chỉ 3 giá trị (user chốt): `Y` = đã có script auto · `N` = auto được nhưng chưa có script · `N/A` = không thể auto. **Automation Result** = kết quả lần chạy script. Từ đó đo **độ phủ automation cho từng bộ test case VÀ cho từng dự án** — KHÔNG phải 1 con số gộp toàn bộ test case.
- **Định nghĩa độ phủ** (mấu chốt): `coverage = Y / (Y + N)`. **N/A bị loại khỏi mẫu số** (không thể auto thì không tính là nợ automation) — nếu để N/A vào mẫu số thì bộ nào nhiều case không-auto-được sẽ bị dìm điểm oan. Ô trống / giá trị lạ → `''` (chưa phân loại), cũng KHÔNG vào mẫu số; hiện riêng để nhắc team điền. `denom = 0` → hiện `—`, không hiện `0%` (0% = đã khai báo mà chưa auto gì, khác hẳn "chưa khai báo").
- **Parse** (`core/testcase_store.py`): `_HEADER_SYNONYMS` thêm field `auto` + `auto_result` (khớp exact theo tên header đã `_norm` nên "Automation Result" KHÔNG nhầm với cột "Result"). `_AUTO_MAP`/`_norm_auto` → `'y'|'n'|'na'|''`; `_norm_auto_result` → `TC_RESULTS` hoặc `''` khi ô trống (chưa chạy script — khác "Not Run" của chấm tay). Case dict thêm `auto`/`auto_result`; `_valid_case` để **optional** (data cũ vẫn valid, không cần migrate). Dòng tràn/merge (spill-over) cũng nhận 2 field này như `result`.
- **LUÔN lấy theo file** mỗi lần import/sync — KHÔNG có logic giữ giá trị cũ như Smart Sync làm với `result` (Decision #42), vì automation không chấm tay trên dashboard. `_get_content_sig` KHÔNG đụng → alignment giữ result vẫn nguyên.
- **UI** (`assets/app_v2.js` closure `#tcBody` + `core/render/testcase.py`): (a) bảng thêm 2 cột **Automated** (badge Y/N/N/A) + **Auto Result** (colgroup/thead/filler/colspan 7→9). ⚠ 9 cột không nhét vừa card ở màn hẹp: cột badge có bề ngang tối thiểu (không bóp được) nên **giữ px cho 4 cột cuối + bọc `.tc-table-wrap{overflow-x:auto}` + `min-width:1240px`** — bản đầu thử ép tất cả về % làm badge "TRUNG BÌNH" bị cắt, còn trộn px+% (tổng >100%) thì 2 cột auto **lòi hẳn ra ngoài viền card** (đúng lỗi user báo); (b) metric card thứ 5 "Độ phủ Automation" (% + `Y/denom`); (c) card chart full-width `autoCoverageCard()` — xem "Tất cả dự án" → bar **theo dự án**, chọn 1 dự án → bar **theo bộ/sheet**, chọn 1 bộ → 1 bar; bar chia 4 segment Y/N/N-A/chưa-phân-loại (tỷ lệ trên TỔNG case của hàng) còn số bên phải là **coverage** (trên mẫu số Y+N); (d) cây Repository mỗi node thêm dòng `% auto` (ẩn khi bộ chưa khai báo Y/N — Decision #55 giữ nguyên dải pass/fail); (e) drawer hiện badge Automated + "Auto: <kết quả>"; (f) **trang `/analytics`** thêm card "Automation Coverage" (`anAutoCovBox`/`anAutoCovRows` + dropdown `#anAutoScope`): KPI tổng + bar so sánh **theo dự án**, chọn 1 dự án → bar **theo bộ/sheet**. Analytics ĐÃ embed sẵn `tcData` (cases+folders) nên **0 call thêm**; công thức tái hiện y hệt trang test-case (`anAutoStats`) — sửa 1 bên phải sửa bên kia.
- **Giới hạn**: giá trị lạ ở cột Automated (vd "Yes-ish") → coi như chưa phân loại (map ở `_AUTO_MAP`, nới thêm synonym nếu team gõ khác). Sheet KHÔNG có cột Automated → mọi case `auto=''` → card hiện `—` (không vỡ). Chưa có metric "pass rate của script auto" (đã có data `auto_result`, dựng sau nếu cần). Cần **restart app** (template Python `render/testcase.py` + store); JS/CSS hot-reload F5 — nhưng phải **sync lại test case** mới có data 2 cột mới.
- **Verify**: `py_compile testcase_store/render.testcase` + `node --check app_v2.js` OK; smoke parse (Y/N/N-A/trống → `y/n/na/''`, `Automation Result` không nhầm cột `Result`, sheet thiếu 2 cột vẫn parse + `valid_store` True); **live browser** (harness tĩnh từ `render_testcase_v2` thật, port 8099): coverage per-bộ 8/15=53% · 2/11=18%, per-dự-án 10/26=38%, dự án không khai báo → `—` + ẩn dòng cây; đổi scope tất-cả→dự-án→bộ ra đúng 3 chế độ bar; drawer + 9 cột bảng đúng; **0 lỗi console**. Layout bảng đo bằng `getBoundingClientRect`: ở 1900px card 1332 / table 1330 → **không cần cuộn**; ở card 712px table giữ 1240px, cuộn ngang TRONG wrapper (body KHÔNG scroll ngang), mọi ô badge `scrollWidth-clientWidth = 0` (hết bị cắt). Analytics: KPI 10/26 = 38.5%, bar theo dự án (Thu Hộ 38.5% · Chi Hộ `—`), chọn Thu Hộ → tách T7 53.3% / T8 18.2%.

### 65. Xem file HTML ngay trong app — iframe `/file-raw` sandbox (2026-07-31)
- **Bối cảnh / vì sao**: viewer tài liệu (Decision #63) dựng lại nội dung server-side cho docx/xlsx/pptx/text, nhưng **file `.html` không có nhánh nào** → rơi vào empty-state "chưa xem trước được". User yêu cầu nhận cả file HTML và render ngay trong hệ thống.
- **KHÔNG parse/dựng lại HTML** (khác docx/xlsx): HTML đã là thứ browser render gốc — bóc text ra `<pre>` thì mất CSS/layout, preview vô nghĩa. Nên **để browser render nguyên bản** trong iframe.
- **Route riêng `GET /file-raw?f=<filename>`** (`routes/uploads.py` `_get_file_raw`, dispatch ở `qa_dashboard`): CHỈ nhận `.html/.htm`, chỉ đọc trong `UPLOADS_DIR` (basename + `resolve().parent` check chống traversal), cap `file_preview.MAX_BYTES` (25MB). **Phải có route riêng** vì `/uploads/...` cố ý serve HTML dạng `application/octet-stream; attachment` (tải về, không render) — nếu đổi `/uploads/` sang `text/html` inline thì file người dùng up sẽ chạy script **trong origin app** = stored XSS ăn session.
- **Cách ly = origin mờ (opaque)**: response gắn `Content-Security-Policy: sandbox allow-scripts allow-popups allow-forms allow-modals` + `X-Content-Type-Options: nosniff` + `Cache-Control: no-store`; client bọc thêm `sandbox="allow-scripts allow-popups allow-forms allow-modals"` + `referrerpolicy="no-referrer"` trên iframe (2 lớp). **KHÔNG có `allow-same-origin`** → verify live: `self.origin === "null"`, `document.cookie`/`localStorage` throw `SecurityError`, `iframe.contentDocument === null`. Vẫn cho `allow-scripts` để report HTML có chart/JS xem được — đánh đổi có chủ đích, an toàn vì origin mờ (cookie SameSite=Lax không gửi kèm request về app) và upload đã gate admin.
- **Client** (`app_v2.js`): `fpOpenUrl(u)` = helper map `/uploads/*.html?` → `/file-raw?f=...` (dùng cho CẢ iframe LẪN nút "Mở tab mới" + context-menu `openLink` — nếu trỏ `/uploads/` thì bấm ra tải file chứ không xem). Nút **Tải xuống** vẫn trỏ `/uploads/...` (đúng: attachment). Nhánh html đặt sau nhánh pdf trong `openDocPreview`. Icon: `getFileIconClass` thêm html/htm → `{code, file-html}`, **nhận cả theo đuôi URL** (tên hiển thị do user đặt thường không có `.html`) + PHMAP `'code':'file-html'` + CSS `.file-html` (cam).
- **`file_preview.py`**: thêm `RAW_EXTS={'.html','.htm'}` → `preview_html` trả `kind='raw'` (nhất quán với `native`/`html`; client không gọi `/file-preview` cho html nhưng giữ để module không tự nhận sai là "unsupported").
- **Giới hạn**: HTML tham chiếu CSS/ảnh/JS **ngoài** (CDN) vẫn tải từ internet (không chặn network trong CSP) — máy không mạng thì hiện thiếu style. File dùng resource **tương đối** (`./style.css`) sẽ đứt vì chỉ up 1 file lẻ vào `uploads/` — cần bản self-contained (inline CSS/JS). `.mhtml`/`.zip` không hỗ trợ. Cần **restart app** (route Python mới); JS/CSS hot-reload F5.
- **Verify**: `py_compile qa_dashboard/routes.uploads/file_preview` + `node --check app_v2.js` OK; header đúng + `?f=../.env` → 400, `?f=*.txt` → 400 (route chỉ html); **live browser** (harness tĩnh từ `render_docs_page` thật, port 8099): iframe `src=/file-raw?...` + sandbox đúng, CSS áp đầy đủ (`body` bg + font + màu h1), script trong file CHẠY nhưng bị cách ly (cookie/localStorage SecurityError, `contentDocument=null`), icon row + head = `file-html`, "Mở tab mới" → `/file-raw`, "Tải xuống" → `/uploads/`, Esc đóng + `fpBody` rỗng (gỡ iframe), file .txt vẫn ra `<pre>` như cũ, **0 lỗi console**.
