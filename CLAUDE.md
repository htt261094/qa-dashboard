...
- **Module `core/chat.py`** — phủ TOÀN BỘ data web bằng **tool-calling + context grounding** (user chốt: model 7B xử lý data kém → cho nó GỌI TOOL lấy số chính xác thay vì tự nhồi/tự đếm):
  ...
  - **Tool-calling** (Ollama `/api/chat` `tools=...`, gemma4 e4b hỗ trợ & gọi chuẩn — đã probe). 4 tool: `count_bugs(project,month,status,person,role)` · `count_testcases(folder,result)` · `count_tasks(assignee,status)` · `find_records(query,kind)`. Đếm = Python lọc full data trả số CHÍNH XÁC (vd PhucTV/T6 → 59, ICCP fail → 389); chi tiết/tìm = `find_records` dùng index retrieval. Schema mô tả tiếng Việt để model nhỏ map đúng.
  ...
  - `_filter_bugs(project,month,status,person,role)` = lõi lọc dùng chung cho `count_bugs` (so khớp lỏng substring/không hoa-thường; month khớp cả tab `T<n>` lẫn ngày tạo). Task Jira chỉ **peek** RAM cache + snapshot đĩa (`_cache_get`/`_snap_read_local`), KHÔNG gọi `fetch_all_shared()` (tránh block ~10s mất VPN). Mọi nguồn lazy-import + soft-fail. Lỗi Ollama → yield câu báo tiếng Việt, KHÔNG raise.
...
- **Config** (`config.py`): `OLLAMA_URL` (default `http://localhost:11434`), `OLLAMA_MODEL` (default `gemma4:e4b-it-qat`), `CHAT_ENABLED=bool(OLLAMA_MODEL)`. Bỏ trống `OLLAMA_MODEL` = tắt tính năng (widget KHÔNG render). Override qua `.env`.
- **Route** `POST /chat` (`_post_chat`, gate authed/domain như mọi POST): nhận `{messages:[{role,content}]}`, stream `text/plain` chunked (KHÔNG Content-Length → connection-close báo hết, HTTP/1.0). `BrokenPipeError` (client đóng tab) nuốt êm. ThreadingHTTPServer (#129) → stream dài KHÔNG đơ request khác.
